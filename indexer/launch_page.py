...
build step, a base58 transaction handed to the wallet, so the page ships no
third-party code a reader would have to trust to know what they are signing.
The wallet is asked to sign only (`signTransaction`) and `api/launch.py`
sends, again and again until the chain has it, because a wallet's own send
was seen to drop a split with no way to resend it (`indexer/relay.py`). A
wallet that will not sign without sending uses `signAndSendTransaction`.
...
function shareRows() {
  ...
  editableRows().forEach(function (row) {
    ...
    if (row.dataset.burn === 'true' && Number.isFinite(pct) && pct < 100) { problems.push('The incinerator share must be at least 1% of the rest.'); b.setAttribute('aria-invalid', 'true'); }
...
async function build() {
  ...
  try {
    ...
    var lines = ['0.25% of each transaction   Charlie Protocol, buys and burns $CHARLIE'];
...
async function waitForConfirmation(second) {
  var end = Date.now() + 120000, signature = second ? state.splitSig : state.createSig, seen = null;
  ...
  try {
    while (Date.now() < end) {
      ...
      // Not landed yet: send the same signed bytes again. The signature is
      // the identity, so a repeat can never make a second transaction.
      if (state.signed) { relay({transaction: state.signed}).catch(function () {}); }
      if (Date.now() < end) { await sleep(Math.min(3000, end - Date.now())); }
    }
    // The chain answered and has never heard of this signature, two minutes
    // after a blockhash that lives about one: it was dropped and cannot land.
    return seen === false ? 'dropped' : 'unknown';
...
function relay(body) {
  return request('/api/launch?send=1', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(body)}, 20000);
}
// The wallet signs; this site sends, and sends again until the chain has it.
// Seen on mainnet 2026-09-17: a wallet's own send returned a signature the
// chain never saw, and nothing could resend it. The server sends only what is
// truly signed (it checks every signature), so whatever shape the wallet
// answers in is passed along as it is. A wallet that cannot sign without
// sending, or answers with something unusable, falls back to its own send.
async function signThenSend(p, built) {
  var res = null;
  state.signed = null;
  try { res = await p.request({method: 'signTransaction', params: {message: built.signable}}); }
  catch (e) { if (e && e.code === 4001) { throw e; } }
  var signed = res && (res.transaction || res.signedTransaction || res.signature || res);
  if (signed && typeof signed.serialize === 'function') { signed = signed.serialize(); }
  if (signed && typeof signed !== 'string' && signed.length !== undefined) { signed = Array.from(signed); }
  if (typeof signed === 'string' || Array.isArray(signed)) {
    for (var attempt = 0; attempt < 3; attempt++) {
      try {
        var d = await relay({transaction: built.transaction, signed: signed});
        state.signed = d.transaction;
        return signatureOf(d.signature);
      } catch (e) {
        // "Not sent" is a verdict on the answer; anything else is the node, so ask again.
        if (/^Not sent/.test(e.message)) { break; }
        await sleep(1500);
      }
    }
  }
  return signatureOf(await p.request({method: 'signAndSendTransaction', params: {message: built.signable}}));
}
...
async function confirmPending() {
  ...
  } else if (outcome === 'dropped' && second) {
    // Nothing was set and the one change is unspent. Build it again; the
    // server simulates first, so a split that did land can never be re-signed.
    timeline('tx2', 'bad', '2. Split never reached the chain', state.splitSig);
    state.splitSig = null;
    await prepareSplit();
    if (state.enroll) { say('sendNote', 'The first split transaction never reached the chain, so nothing was set. A fresh one is checked and ready. Check the summary above, then sign approval 2.', 'caution'); }
  } else if (outcome === 'dropped') {
    state.locked = false; state.phase = 'draft'; state.createSig = null; invalidate(false);
    say('sendNote', 'The create transaction never reached the chain. No coin was created. Check and build again.', 'caution');
  } else {
    $('checkAgain').hidden = false;
...
async function send() {
  ...
  try {
    var sig = await signThenSend(p, second ? state.enroll : state.built);
...
<main>
  ...
  <section class="launch-checklist step-list" aria-labelledby="checklistTitle">
    ...
    <div class="blueprint-flow-grid">
      ...
      <div class="blueprint-flow-item" data-reveal><span class="blueprint-flow-num">03</span>
        ...
        <p class="blueprint-flow-desc">The protocol receives 0.25% of each transaction. Only the incinerator row destroys SOL. That row is fixed: you choose its share, 1% of the rest or more, and other destinations receive theirs.</p></div>
  ...
  <section class="cyber-card cyber-card--chamfer launch-card" id="splitBox" data-edit aria-labelledby="splitTitle">
    ...
    <p><strong>pump allows exactly one split change. This split cannot be changed after it is set.</strong>
      ...
      The incinerator row is fixed and SOL paid to it is destroyed; you choose its share, 1% of the rest or more. Other wallets receive SOL.</p>
    <div class="share-row locked" role="group" aria-label="Fixed protocol share">
      ...
      <input id="protocolBps" type="hidden">
    </div>
    <div id="shares"></div>
    ...
    <div class="chain-columns">
      <div><h3>First, pump creates the coin</h3>
        <p>pump’s original <code>create</code> instruction makes the coin, its metadata and its bonding curve—the account pump uses to price trades—with your wallet as creator.
          Your wallet signs alongside a mint keypair the server made ahead of time, chosen so the coin’s address ends in <code>1nc1n</code>. That key signs creation once; pump’s mint authority takes over when the instruction runs.</p>
        <p>The ending is a mark, not proof: anyone can make an address that ends the same way. The split on the chain is the proof.</p>
