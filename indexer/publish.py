...
NON_FIGURE_EMITTERS = {
    ...
    "indexer.protocol_burns:main": (
        ...
        "No name in invariants.FIGURES passes through it: it is deliberately NOT "
        "$CHARLIE's supply destroyed, which the record withholds because "
        "BURN_SUPPLY gates it, and it is one wallet's own spending on a mint that "
        "cannot enroll, so it is not that coin's BURN_TOTAL either. "
        "That alone was not enough, because being outside FIGURES is not the same "
        "as being checkable: the walk skips a signature the node will not return "
        "and asks for one page of history, so the total could understate with "
        "nothing able to say so. It now carries its own completeness -- "
        "protocol_burns.Coverage, asserted by tests/test_protocol_burns.py -- and "
        "every record states whether its total is `exact` or `at least`, and why. "
        "A consumer that reads nothing else cannot take an understated number for "
        "an exact one"
    ),
