...
```
init_vault(mint)   -> creates sol_burn_vault = PDA(["sol_burn", mint]) and burn_pool = PDA(["burn", mint])
...
lamports out of a `sol_burn` PDA. Anyone can verify that by reading it.
...
SOL-burn or token-burn vault. `indexer/legs.py` holds `PROGRAM_ID = None` for exactly that
