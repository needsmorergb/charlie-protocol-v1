    ...
    accounts = {
        ...
        config_addr: config_account(
            ...
            [(site_legs.TOLL_DESTINATION, site_legs.TOLL_BPS),
             ...
             (site_legs.LAUNCH_BUYBACK_DESTINATION, 500),
             ("9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM",
              10000 - site_legs.TOLL_BPS - 2000 - 500)],
            admin_revoked=True,
        ),
...
class TestTheEnrolmentRate(unittest.TestCase):
    """`site.py` imports no fee module, so it carries its own copy of the
    value a config must pay. This is what stops the page from advertising a
    rate the crank no longer charges."""
    ...
    def test_the_landing_page_states_the_rate_one_way(self):
        """0.25% of each transaction, and never as a share of the creator fee
        or as a figure derived against a trade."""
        page = site.render_landing(_counters_fixture(), now=1)
        self.assertIn(site.TOLL_RATE_SENTENCE, page)
